...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($title) && $customer_id > 0) {
        $stmt = $conn->prepare("INSERT INTO sales (customer_id, title, description, amount, currency, status, probability, expected_close_date, assigned_to, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("issdssissi", $customer_id, $title, $description, $amount, $currency, $status, $probability, $expected_close_date, $assigned_to, $_SESSION['user_id']);
        
        if ($stmt->execute()) {
            $sale_id = $stmt->insert_id();
            logActivity($_SESSION['user_id'], 'create', 'sale', $sale_id, "Yeni satış eklendi: $title");
            $success = 'Satış başarıyla eklendi!';
        } else {
            $error = 'Satış eklenirken bir hata oluştu!';
        }
        $stmt->close();
    } else {
...
<div class="row mb-4">
    <div class="col-md-6">
        <h1><i class="bi bi-plus-circle"></i> Yeni Satış Ekle</h1>
    </div>
        ...
        <form method="POST" action="">
            ...
            <div class="row">
                ...
                <div class="col-md-3 mb-3">
                    ...
                    <input type="number" class="form-control" name="probability" value="50" min="0" max="100">
                </div>
            </div>
